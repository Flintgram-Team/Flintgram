...
public class SharedConfig {
    ...
    public static int keepMedia = CacheByChatsController.KEEP_MEDIA_ONE_MONTH;     public static int lastKeepMediaCheckTime;
    ...
    private static final int[] LOW_SOC = {
            -1775228513,             802464304,              802464333,              802464302,              2067362118,             2067362060,             2067362084,             2067362241,             2067362117,             2067361998,             -1853602818     };
    ...
    public static void setHideReadReceipts(boolean value) {
        ...
        try {
            ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Context.MODE_PRIVATE)
                    .edit()
                    .putBoolean("hideReadReceipts", hideReadReceipts)
                    .apply();
        } catch (Exception e) {
            FileLog.e(e);
        }
    ...
    public static void setPlumFreeVoiceTranscription(boolean value) {
        if (plumFreeVoiceTranscription == value) {
            return;
        }
        plumFreeVoiceTranscription = value;
        try {
            ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Context.MODE_PRIVATE)
                    .edit()
    ...
    public static void setDeletedMessageStyle(int value) {
        if (deletedMessageStyle == value) {
            return;
        }
        deletedMessageStyle = value;
        try {
            ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Context.MODE_PRIVATE)
                    .edit()
    ...
    public static void markMessageDeleted(long dialogId, int messageId) {
        if (messageId == 0) {
            return;
        }
        String key = dialogId + "_" + messageId;
        if (!deletedMessages.add(key)) {
            return;
        }
        try {
            ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Context.MODE_PRIVATE)
                    .edit()
                    .putStringSet("plumDeletedMessages", new HashSet<>(deletedMessages))
                    .apply();
        } catch (Exception e) {
            FileLog.e(e);
        }
    ...
    public static boolean isMessageDeleted(long dialogId, int messageId) {
        return deletedMessages.contains(dialogId + "_" + messageId);
    }
    ...
    public static void loadProxyList() {
        ...
        if (!TextUtils.isEmpty(list)) {
            ...
            if (count == -1) {                 int version = data.readByte(false);
