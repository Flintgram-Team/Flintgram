...
public class SettingsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate, ImageUpdater.ImageUpdaterDelegate, MainTabsActivity.TabFragmentDelegate, FactorAnimator.Target {
    ...
    public View createView(Context context) {
        ...
        listView.setSections(dp(12), dp(25), false);
    ...
    public void openDebugMenu() {
        ...
        builder.setItems(items, (dialog, which) -> {
            if (which == 0) {                 getUserConfig().syncContacts = true;
                getUserConfig().saveConfig(false);
                getContactsController().forceImportContacts();
            } else if (which == 1) {                 getContactsController().loadContacts(false, 0);
            } else if (which == 2) {                 getContactsController().resetImportedContacts();
            } else if (which == 3) {                 getMessagesController().forceResetDialogs();
            } else if (which == 4) {                 BuildVars.LOGS_ENABLED = !BuildVars.LOGS_ENABLED;
            ...
            } else if (which == 5) {                 SharedConfig.toggleInappCamera();
            } else if (which == 6) {                 getMessagesStorage().clearSentMedia();
                ...
                editor.apply();
            } else if (which == 7) {                 VoIPHelper.showCallDebugSettings(getParentActivity());
            } else if (which == 8) {                 SharedConfig.toggleRoundCamera16to9();
            } else if (which == 9) {                 ((LaunchActivity) getParentActivity()).checkAppUpdate(true, null);
            } else if (which == 10) {                 getMessagesStorage().readAllDialogs(-1);
            } else if (which == 11) {                 SharedConfig.toggleDisableVoiceAudioEffects();
            } else if (which == 12) {                 SharedConfig.pendingAppUpdate = null;
                SharedConfig.saveConfig();
                NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.appUpdateAvailable);
            } else if (which == 13) {                 Set<String> suggestions = getMessagesController().pendingSuggestions;
                suggestions.add("VALIDATE_PHONE_NUMBER");
                suggestions.add("VALIDATE_PASSWORD");
                getNotificationCenter().postNotificationName(NotificationCenter.newSuggestionsAvailable);
            } else if (which == 14) {                 ApplicationLoader.applicationContext.deleteDatabase("webview.db");
                ...
                cookieManager.flush();
            } else if (which == 16) {                 SharedConfig.toggleDebugWebView();
            ...
            } else if (which == 17) {                 SharedConfig.toggleForceDisableTabletMode();
                Activity activity = getParentActivity();
                if (activity != null) {
                    final PackageManager pm = activity.getPackageManager();
                    final Intent intent = pm.getLaunchIntentForPackage(activity.getPackageName());
                    activity.finishAffinity();                     activity.startActivity(intent);                 }
                System.exit(0);
            } else if (which == 18) {
